# comot
Node.js library for reading a file (local path, buffer, string/base64, stream, url) and then store it anywhere (local, Google Cloud Storage, AWS S3, Cloudinary, and many more will come) at anytime.

## Installation
```bash
$ npm install --save comot
```

## Usage
```js
const Storage = require('comot');
```

### Start Reading
Reading from local path (file)
```js
const Input = Storage.Reader('./image.jpg');
```
Reading from url
```js
const Input = Storage.Reader('https://www.google.co.id/images/branding/googlelogo/1x/googlelogo_color_272x92dp.png');
```
Reading from base64 string
```js
const Input = Storage.Reader('://www.google.co.id/images/branding/googlelogo/1x/googlelogo_color_272x92dp.');
```
Reading from buffer
```js
const Input = Storage.Reader(<Buffer>);
```
Reading from stream
```js
const Input = Storage.Reader(<Stream>);
```
### Converting
Convert to buffer
```js
Input.toBuffer().then(function (buff) {
    // now in buffer
    console.log(buff);
}).catch(console.error);
```
Convert to stream
```js
Input.toStream().then(function (readStream) {
    // now in Readable Stream, piping it to writeable stream will be possible
    const writeStream = require('fs').createWriteStream('./copy-image.jpg');

    readStream.pipe(writeStream);
}).catch(console.error);
```
Convert to file
```js
Input.toFile('./another-copy.jpg').then(function (path) {
    // save as another file
    console.log(path);
}).catch(console.error);
```
Convert to base64 string
```js
Input.toString().then(function (string) {
    // now in string
    console.log(string);
}).catch(console.error);
```
### Uploading
Upload to local
```js
const Output = Storage.Writer('local');

Output.upload(Input, './image-copy.jpg').then(console.log).catch(console.error);
```
Upload to Cloudinary
```js
const Output = Storage.Writer('cloudinary', {
    cloud_name: '... your cloudinary cloud name',
    api_key: '... your cloudinary api key',
    api_secret: '... your cloudinary api secret',
    secure: true // use https 
});

Output.upload(Input, './image.jpg').then(console.log).catch(console.error);
```
Upload to Google Cloud Storage
```js
const Output = Storage.Writer('google-cloud-storage', {
    keyFile: '... your json file'
});

Output.upload(Input, 'image.jpg', '... your bucket name').then(console.log).catch(console.error);
```

Upload to AWS S3
```js
const Output = Storage.Writer('aws-s3', {
    accessKeyId: '... your aws access key id',
    secretAccessKey: '... your aws access key secret',
    region: '... your s3 region'
});

Output.upload(Input, 'image.jpg', '... your bucket name').then(console.log).catch(console.error);
```